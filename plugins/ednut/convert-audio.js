module.exports = [
  {
    ...
    ban: true,
    gcban: true,
    execute: async (m, { ednut, quoted, mime, toAudio }) => {
      if (!quoted) return m.reply("Reply to a video or audio");
      if (!/video/.test(mime) && !/audio/.test(mime)) return m.reply("Reply to a video or audio");
  ...
  },
  {
    ...
    ban: true,
    gcban: true,
    execute: async (m, { ednut, quoted, mime, toPTT }) => {
      if (!quoted) return m.reply("Reply to a video or audio");
      if (!/video/.test(mime) && !/audio/.test(mime)) return m.reply("Reply to a video or audio");
      let media = await quoted.download();
  ...
  ban: true,
  gcban: true,
  execute: async (m, { ednut, quoted, mime, command, getRandom, fs, exec }) => {
    if (!quoted) return m.reply("Reply to an audio");
